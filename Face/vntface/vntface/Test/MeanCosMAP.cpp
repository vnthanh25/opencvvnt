...
void aMeanCosVNTDatabaseInit()
{
	...
	vFaceTrackDB.aDatabaseInit(vFaceDataSet.aGetFaceTraks(), vFaceTracksPath);
	...
	*/
	...
	for (size_t i = 0; i < n; i++)
	{
		...
		for (size_t j = 0; j < m; j++)
		{
			...
			std::vector<std::vector<double>> vAvgFeatureQuery = vFaceTrackDB.aAvgFeature(vFeaturesQuery);
