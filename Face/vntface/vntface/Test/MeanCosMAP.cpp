...
void aAvgFaceTrackAll()
{
	...
	std::vector<std::vector<std::vector<double>>> avgfeaturefacetrack = vFaceTrackDB.aGetsAvgFeatureFake(3, 3, 3);
	//\\ Tinh vector trung binh cho tat ca facetrack.
	std::vector<std::vector<double>> avgFeature = vFaceTrackDB.aAvgFeature(avgfeaturefacetrack);

	//\\ Gia lap: moi factrack co nhieu vector dac trung.
	std::vector<std::vector<std::vector<std::vector<int>>>> avgfeaturefacetrack1 = vFaceTrackDB.aGetsFeaturesFake(3, 3, 3, 3);
	//\\ Tinh vector trung binh cho tat ca facetrack.
	std::vector<std::vector<double>> avgFeature1 = vFaceTrackDB.aAvgFeature(avgfeaturefacetrack1);
}
...
void aCosineTwoVector()
{
	FaceTrackDB vFaceTrackDB;
	//\\ Gia lap 2 vector.
	std::vector<std::vector<double>> vector1 = vFaceTrackDB.aAvgFetureFake(1, 2, 1);
	std::vector<std::vector<double>> vector2 = vFaceTrackDB.aAvgFetureFake(1, 2, 2);
	//\\ Tinh khoang cach cosine cua 2 vector.
	double cosine = vFaceTrackDB.aCosine(vector1, vector2);
}
...
void aEuclidTwoVector()
{
	...
	std::vector<std::vector<double>> vector2 = vFaceTrackDB.aAvgFetureFake(1, 2, 2);
	//\\ Tinh khoang cach cosine cua 2 vector.
	double cosine = vFaceTrackDB.aEuclid(vector1, vector2);
}
...
void aMatchingFaceTrack()
{
	...
	std::vector<std::vector<std::vector<std::vector<int>>>> facetracks = vFaceTrackDB.aGetsFeaturesFake(numFaceTrack, numFeature, numRow, numCol);
}
...
void aMeanCosVNTDataSetInit()
{
	...
	vFaceTrackDB.aDatabaseInit(vFaceDataSetQuery.aGetFaceTraks(), vFaceTracksPath);

	//\\ Hien thi thoi gian ket thuc.
	cout << util.currentDateTime() << std::endl;
}
//\\ Khoi tao csdl
void aMeanCosHeadPoseInit(FaceDataSet pFaceDataSet, FaceTrackDB pFaceTrackDB, std::string pDataSourcePath, std::string pFolderPath)
{
	Utilites util;
	//\\ Hien thi thoi gian bat dau.
	cout << "aMeanCosHeadPoseInit:" << std::endl;
	...
	//\\ Khoi tao csdl.
	pFaceTrackDB.aDatabaseInit(pFaceDataSet.aGetFaceTraks(), pFolderPath);

	//\\ Hien thi thoi gian ket thuc.
	cout << util.currentDateTime() << std::endl;
}
//\\ So khop facetrack
std::vector<std::vector<cv::Mat>> aMeanCosHeadPoseMatching(std::vector<cv::Mat> pFaceTrackQuery, std::vector<std::vector<cv::Mat>> pFaceTracks, FaceTrackDB pFaceTrackDB)
{
	...
	std::vector<std::vector<std::vector<int>>> vFeaturesQuery = pFaceTrackDB.aGetsFeature(pFaceTrackQuery);
	std::vector<std::vector<double>> vAvgFeatureQuery = pFaceTrackDB.aAvgFeature(vFeaturesQuery);
	...
	std::vector<int> vFaceTracksMatchingIndex = pFaceTrackDB.aMeanCosMatchingIndex(vQuery, csdl);
...
//\\ So khop facetrack
std::vector<int> aMeanCosHeadPoseMatchingIndex(std::vector<cv::Mat> pFaceTrackQuery, FaceTrackDB pFaceTrackDB)
{
	...
	std::vector<std::vector<std::vector<std::vector<double>>>> csdl = pFaceTrackDB.aGetFaceTrackDatabase();
	//\\ So khop facetrack truy van va cac facetrack trong csdl.
	result = pFaceTrackDB.aMeanCosMatchingIndex(vQuery, csdl);
	return result;
}
...
void aMeanCosHeadPose()
{
	...
	std::vector<std::vector<cv::Mat>> facetracksMatching = aMeanCosHeadPoseMatching(facetracksquery[0], facetracks, vFaceTrackDB);
}
...
double aMeanCosHeadPoseMAP1()
{
	...
	*/
	...
	for (size_t i = 0; i < n; i++)
	{
		...
		{
			std::vector<int> vMatchingIndex = aMeanCosHeadPoseMatchingIndex(facetracks[i * m + j], vFaceTrackDB);
			double vAP = 0;
			double Nr = 1;
...
double aMeanCosHeadPoseMAP()
{
	...
	*/
	...
	for (size_t i = 0; i < n; i++)
	{
		...
		for (size_t j = 0; j < m; j++)
		{
			std::vector<int> vMatchingIndex = vFaceTrackDB.aMeanCosMatchingIndex(csdl[i * m + j], csdl);
